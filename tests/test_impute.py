...
def test_output():
    ...
    assert len(summary['missing_indeces_']['train_set']
               ) != 0, 'If imputation performed, then indeces with missing values should be present'

    assert len(summary['best_imputed_data_']
               ) == 2, 'imputed X_train and X_valid are not present'

    assert len(summary['imputation_scores_']
               ) == 6, 'scores for all methods not present'
